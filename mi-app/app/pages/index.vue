              ...
              <div class="grid lg:grid-cols-[minmax(0,1.2fr),minmax(260px,0.9fr)] gap-8 items-start">
                <div class="space-y-5">
                  <p class="text-sm text-slate-200/90">
                    Usamos el reglamento oficial de Flag Football. Aquí tienes un resumen rápido
                    para que jugadores y coaches tengan claro cómo se juega la liga.
                  </p>
                ...
                <div class="space-y-4 rounded-2xl bg-slate-800/70 border border-slate-700/70 p-4">
                  ...
                  <div class="mt-3 rounded-xl bg-slate-900/60 border border-slate-700 px-3 py-3 text-[11px] text-slate-300 space-y-1.5">
                    ...
                    <p>
                      Antes del primer juego, comparte este resumen y el PDF completo en el chat de tu equipo.
                      Así todos llegan con el mismo entendimiento del reglamento.
                    </p>
                  </div>
                </div>
              </div>
...
const ruleSections = ref<RuleSection[]>([
  {
    ...
    bullets: [
      ...
      'Objetivo: avanzar el balón y sumar más puntos que el rival.',
    ],
  },
  {
    id: 'campo-tiempo',
    ...
    bullets: [
      ...
      'Partido en dos mitades, con reloj controlado por oficiales.',
      'El reloj puede detenerse en situaciones clave.',
    ],
  },
  {
    id: 'equipamiento',
    ...
    bullets: [
      'Cinturón de flags visible; jersey por dentro del short.',
      'Calzado adecuado y, de ser posible, protector bucal.',
      'Prohibidos accesorios peligrosos (joyería expuesta, objetos duros, etc.).',
    ],
  },
  {
    id: 'conducta',
    ...
    bullets: [
      'Sin contacto: no hay bloqueos ni tacleos.',
      'Se sancionan acciones antideportivas y contacto ilegal.',
      'Capitanes: responsables de la comunicación con oficiales.',
    ],
  },
])
...
const activeRule = computed<RuleSection>(() => {
  const found = ruleSections.value.find((s) => s.id === activeRuleId.value)
...
onMounted(() => {
  if (heroSlides.value.length > 1) heroIntervalId = setInterval(nextSlide, 7000)
