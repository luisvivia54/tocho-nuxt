<template>
  <div class="bg-[#020617] text-slate-50 min-h-screen">
    ...
    >
      <nav
        class="max-w-6xl mx-auto container-pad px-6 py-4 flex items-center justify-between gap-4"
      >
        <!-- LOGO -->
...
const isAdmin = computed<boolean>(() => {
  const v = authz?.isAdmin ?? authz?.isAdminRole
  if (typeof v === 'boolean') return v
  if (v && typeof v === 'object' && 'value' in v) return !!v.value

  const roles = authz?.realmRoles ?? authz?.roles ?? authz?.tokenRoles
...
const handleAuth = () => {
  // Preferimos los helpers del composable si existen
  if (isAuthenticated.value) {
    if (authz?.logout) return authz.logout()
  } else {
    if (authz?.login) return authz.login()
  }

  // Fallback: si por alguna razón no existen, intentamos con $kc
  const nuxtApp = useNuxtApp()
  const kc = (nuxtApp as any).$kc
  if (!kc) return
  return isAuthenticated.value ? kc.logout() : kc.login()
}

// ----- Sync /api/me (si lo usas para tu backend) -----
