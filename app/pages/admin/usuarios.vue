...
<template>
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6">
          <div class="flex flex-col gap-3 py-4 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex flex-wrap items-center gap-2">
              <button
                class="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-sm font-semibold text-slate-100 hover:bg-white/10 hover:text-white focus:outline-none focus:ring-2 focus:ring-cyan-400/40"
                @click="fakeSync"
                title="Maqueta"
              >
              ...
              <div class="flex flex-col gap-2 border-b border-white/10 p-4 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p class="text-xs text-slate-400">
                    {{ filteredSorted.length }} resultados · Página {{ page }} / {{ totalPages }}
                  </p>
                </div>
                <div class="flex items-center gap-2">
                  <button
                    class="rounded-2xl border border-white/10 bg-black/20 px-3 py-2 text-xs font-semibold text-slate-200 hover:bg-black/30 focus:outline-none focus:ring-2 focus:ring-white/20"
            ...
            <Transition name="slide">
              <aside v-if="selectedUser" class="sticky top-[92px] rounded-3xl border border-white/10 bg-white/5">
                ...
                <div class="p-4 space-y-4">
                  ...
                  <div class="rounded-3xl border border-white/10 bg-black/20 p-4">
                    <p class="text-sm font-semibold text-white">Regla rápida (opcional)</p>
                    <p class="mt-2 text-xs text-slate-300">
                      Si bajas a <b>Usuario</b> y tienes más de 1 equipo, esta vista lo recorta a 1 automáticamente.
                      (Quita esa regla si no la quieres).
                    </p>
                  </div>
                </div>
              </aside>
  ...
  /** Fila "tal cual" de tu BD/CSV: role viene como string */
  type UserRow = {
    id: number
    keycloakId: string
    email: string
  ...
  type User = {
    ...
    // precomputados (mejor scroll/renders)
    initials: string
    avatarStyle: Record<string, string>
  }
  
  /* ✅ Seed (reemplázalo por tu fetch cuando quieras) */
  const seedRows: UserRow[] = [
    {
      id: 1,
      keycloakId: 'af650af7-e349-4a37-a04a-975dc2c0b816',
      email: 'luisvivia54@gmail.com',
      fullName: 'Luis Vivia',
      role: 'ADMIN',
      maxTeamsAllowed: 10,
      isActive: true,
      createdAt: '2025-11-30T16:02:13.183418Z',
      updatedAt: '2025-12-09T21:26:05.005263Z',
    },
    {
      id: 2,
      keycloakId: '7b9e3d37-52ad-41bd-82d1-3efea0e0d0ff',
      email: 'gustavo@tochero5.mx',
      fullName: 'Gustavo González',
      role: 'USER',
      maxTeamsAllowed: 1,
      isActive: true,
      createdAt: '2025-12-01T03:10:02.120000Z',
      updatedAt: '2025-12-09T21:26:05.005263Z',
    },
    {
      id: 3,
      keycloakId: '4c7b9f11-7f0d-4f4f-a502-1965a9c7d155',
      email: 'mariaf@club.mx',
      fullName: 'María Fernanda',
      role: 'CAPTAIN',
      maxTeamsAllowed: 2,
      isActive: true,
      createdAt: '2025-12-27T18:10:00.000000Z',
      updatedAt: '2025-12-27T18:10:00.000000Z',
    },
  ]
  
  const users = ref<User[]>(seedRows.map(rowToUser))
  ...
  const filteredSorted = computed(() => {
    ...
    if (roleOnly.value !== 'all') list = list.filter(u => u.role === roleOnly.value)
  
    if (q) {
      list = list.filter(u => {
        const hay = `${u.fullName} ${u.email} ${u.keycloakId} ${roleLabel(u.role)}`.toLowerCase()
    ...
    switch (sortKey.value) {
      case 'nameAsc':
        ...
        break
      case 'teamsDesc':
        list.sort((a, b) => b.maxTeamsAllowed - a.maxTeamsAllowed)
        break
      default:
  ...
  function applyChanges() {
    ...
    // Admin no cambia rol en esta vista
    if (u.role !== 'ADMIN') {
      u.role = roleDraft.value === 'CAPTAIN' ? 'CAPTAIN' : 'USER'
    } else {
    ...
    // ✅ regla opcional: Usuario máximo 1
    if (u.role === 'USER' && nextTeams > 1) {
      nextTeams = 1
      teamsDraft.value = 1
    ...
    // refresca campos UI si cambia algo relevante
    refreshUi(u)
  
    dirtyIds.value = new Set(dirtyIds.value).add(u.id)
    toast('Cambios aplicados (mock)', 'success')
  }
  ...
  function fakeSync() {
    ...
    toast(n ? `Guardado (mock): ${n} cambio(s)` : 'Nada que guardar', n ? 'success' : 'info')
    // aquí conectas tu PUT/PATCH real; al success: dirtyIds.value = new Set()
  }
  ...
  /* -------- perf: precompute avatar + initials -------- */
  ...
  /* ✅ Normaliza rol string (venga como venga) a RoleDb */
  ...
  /* Convierte fila BD/CSV a User UI */
  function rowToUser(r: UserRow): User {
    ...
    return {
      ...
      flags: { verified: role === 'ADMIN' }, // ajusta si tienes un campo real de verificación
      initials,
      avatarStyle: style,
    }
  }
  ...
  const stats = computed(() => {
    ...
    return [
      ...
      { label: 'Capitanes', value: users.value.filter(u => u.role === 'CAPTAIN').length, hint: 'Pueden crear equipos', iconPath: icon.captain },
  ...
  /* Mejor scroll cuando hay muchas filas */
  .cv-auto {
