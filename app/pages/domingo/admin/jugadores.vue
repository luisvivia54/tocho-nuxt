<template>
  <main class="min-h-screen bg-[#050816] text-slate-100">
    <section class="pt-6 md:pt-8 lg:pt-10">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 pb-10">
        <!-- Header -->
        <header class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div class="min-w-0">
            <p class="text-[11px] uppercase tracking-[0.22em] text-sky-300/80">
              Consola Admin · Liga Dominical
            </p>
          ...
          <div class="flex flex-wrap items-center gap-2">
            <button
              type="button"
              class="inline-flex items-center justify-center rounded-xl border border-sky-400/30 bg-sky-400 px-4 py-2 text-xs font-semibold text-[#0a1426] transition hover:bg-sky-300"
              @click="refreshAll"
            >
              ⟳ Refrescar
            </button>
          ...
          <div class="grid grid-cols-1 gap-3 md:grid-cols-12">
            ...
            <div class="md:col-span-4">
              ...
              <div class="relative">
                <select
                  v-model="teamPick"
                  class="w-full rounded-xl border border-white/10 bg-[#0B1020] px-3 py-2 text-sm text-slate-100 outline-none transition focus:border-sky-400/60"
            ...
            <div class="md:col-span-4">
              ...
              <div class="relative">
                ...
                <input
                  ...
                  class="w-full rounded-xl border border-white/10 bg-[#0B1020] py-2 pl-8 pr-3 text-sm text-slate-100 outline-none transition placeholder:text-slate-500 focus:border-sky-400/60"
                />
              ...
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0 flex items-start gap-3">
                  <div class="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-2xl border border-white/10 bg-white/5">
                    ...
                    />
                    <span v-else class="text-[12px] font-extrabold text-sky-300">
                      {{ initials(p.fullName) }}
                    </span>
                  </div>
                  ...
                  <div class="min-w-0">
                    ...
                    <div class="mt-2 flex flex-wrap gap-2 text-[11px]">
                      <span class="inline-flex items-center rounded-full bg-sky-400/10 px-2 py-1 font-semibold text-sky-200 ring-1 ring-sky-400/20">
                        {{ p.code || '—' }}
                      </span>
            ...
            <table class="min-w-[1280px] w-full text-sm">
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-3 py-3">
                    <span class="inline-flex items-center rounded-full bg-sky-400/10 px-2 py-0.5 text-xs font-semibold text-sky-200 ring-1 ring-sky-400/20">
                      {{ p.code || '—' }}
                    </span>
                  </td>
        ...
        <aside class="absolute right-0 top-0 h-full w-full border-l border-white/10 bg-[#0A0F1E] shadow-2xl sm:w-[520px]">
          <div class="flex h-full flex-col">
            ...
            <div class="flex items-center justify-between gap-3 border-t border-white/10 p-5">
              ...
              <button
                class="rounded-xl border border-sky-400/30 bg-sky-400 px-4 py-2 text-xs font-semibold text-[#0a1426] transition hover:bg-sky-300 disabled:opacity-50"
...
const {
  ...
  'admin-domingo-players-categories',
  async () => {
...
const teamsVm = computed<TeamVM[]>(() => {
  ...
  for (const x of list) {
    ...
    const leagueOk = leagueId === null || leagueId === DOMINGO_LEAGUE_ID
...
async function downloadCurpPdf() {
  try {
    ...
    doc.setFontSize(16)
    doc.text(`Listado de CURP (Jugadores · Domingo Admin · Liga ${DOMINGO_LEAGUE_ID})`, 40, 48)
    ...
    doc.save(`CURP_Domingo_L${DOMINGO_LEAGUE_ID}_${namePart}_${dateStr}.pdf`)
...
function downloadCurpExcel() {
  try {
    ...
    const namePart = sanitizeFileName(teamPick.value !== 'ALL' ? teamPickLabel.value : 'Todos')
    const fileName = `CURP_Domingo_L${DOMINGO_LEAGUE_ID}_${namePart}_${dateStr}.csv`
