<script setup lang="ts">
  ...
  interface TeamDetail {
    // tu API a veces manda teamId, a veces id
    teamId?: number
    id?: number
    ...
    // ✅ colores (lo que quieres)
    colorPrimary?: string | null
    colorSecondary?: string | null
  
    // campos extra si tu endpoint los incluye (no estorban)
    seasonId?: number
    categoryId?: number
  }
  ...
  // ✅ Ahora sí: colores reales del equipo (se cargan del GET y se mandan al PUT)
  ...
  const activePhotosCount = computed(() => photos.value.filter((p) => !p.markedForDeletion).length)
  ...
  // ✅ Convierte "pink" / "blue" / "rgb(...)" / "#abc" -> "#rrggbb"
  // Esto es SOLO para que el <input type="color"> no se rompa.
  ...
  const rgb = getComputedStyle(el).color // "rgb(255, 192, 203)"
...
function normalizeColorForPicker(raw?: string | null, fallback = '#1D4ED8'): string {
  const v = (raw ?? '').trim()
  ...
  const loadTeam = async () => {
    ...
    try {
      ...
      // ✅ conectar colores
      colorPrimary.value = normalizeColorForPicker(data.colorPrimary, '#1D4ED8')
      colorSecondary.value = normalizeColorForPicker(data.colorSecondary, '#FFFFFF')
    } catch (err) {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      // 1) Actualizar datos básicos del equipo (✅ incluye colores)
      await $fetch(`/teams/${teamId.value}`, {
        ...
        body: {
          ...
          // ✅ aquí van los colores ya en HEX (porque el picker trabaja en HEX)
      ...
      for (const p of players.value) {
        ...
        if (p.photoFile) form.append('photo', p.photoFile)
      ...
      for (const p of players.value) {
        ...
        try {
          ...
          if (status === 413) {
            errorMessage.value = 'Una de las fotos actualizadas de jugador es demasiado pesada. Usa imágenes más ligeras.'
          } else {
      ...
      const existingActivePhotos = photos.value.filter((p) => p.photoId && !p.markedForDeletion).length
      ...
      for (const photo of photos.value) {
        ...
        try {
          ...
          if (status === 413) {
            errorMessage.value = 'Una de las fotos nuevas del carrusel es demasiado pesada. Usa imágenes más ligeras.'
          } else {
      ...
      const rawMessage: string | undefined = err?.data?.message ?? err?.response?._data?.message ?? err?.message
      ...
      if (rawMessage && rawMessage.includes('uq_team_league_name')) {
        ...
        errorMessage.value = 'Ocurrió un error al guardar los cambios. Intenta de nuevo más tarde.'
      }
    } finally {
      saving.value = false
    }
            ...
            <form class="px-6 py-8 space-y-8" @submit.prevent="onSubmit">
              ...
              <div class="space-y-4">
                ...
                <p class="text-xs text-slate-400">
                  Nombre, identificadores, colores y logo del equipo.
                </p>
              ...
              <div class="space-y-4 mt-8">
                <div class="flex items-center justify-between gap-2">
                  <div>
                    <h3 class="font-semibold text-slate-50 text-lg">Fotos del carrusel</h3>
                    <p class="text-xs text-slate-400">
                ...
                <p class="text-[11px] text-slate-500">Fotos activas: {{ activePhotosCount }} / 5</p>
                ...
                <div v-else class="space-y-3">
                  <div
                    ...
                    :class="[
                      ...
                      photo.markedForDeletion ? 'border-red-500/70 bg-red-950/30' : 'border-slate-700 bg-slate-900/60'
                    ]"
                  >
                    <div class="flex items-start justify-between gap-3 mb-3">
                      <div class="flex items-center gap-2 text-xs text-slate-400">
                        <span class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-slate-800 text-[11px] font-semibold text-slate-50">
                          {{ index + 1 }}
                        </span>
                        <span
                    ...
                    <div class="grid md:grid-cols-[minmax(0,2fr)_minmax(0,1.5fr)] gap-4 items-start">
                      ...
                      <div class="w-full h-24 rounded-2xl border border-dashed border-slate-700 bg-slate-900 flex items-center justify-center overflow-hidden">
                        <img
                ...
                <p class="text-[11px] text-slate-500">
                  Las fotos marcadas como <span class="text-red-300">"Se eliminará"</span> se borrarán al guardar.
                </p>
              </div>
              ...
              <div class="space-y-4 mt-8">
                ...
                <div class="space-y-3">
                  <div
                    ...
                    :class="[
                      'rounded-2xl px-4 py-4 sm:px-5 sm:py-4 border',
                      player.markedForDeletion ? 'border-red-500/70 bg-red-950/30' : 'border-slate-700 bg-slate-900/60'
                    ]"
                  >
                    <div class="flex items-start justify-between gap-3 mb-3">
                      <div class="flex items-center gap-2 text-xs text-slate-400">
                    ...
                    <div class="grid md:grid-cols-[minmax(0,3fr)_minmax(0,2fr)] gap-4 items-start">
                      ...
                      <div class="space-y-2">
                        ...
                        <div class="mt-2 w-full h-24 rounded-2xl border border-dashed border-slate-700 bg-slate-900 flex items-center justify-center overflow-hidden">
                          <img
