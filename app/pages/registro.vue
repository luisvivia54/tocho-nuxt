...
const onSubmit = async () => {
  ...
  try {
    ...
    // 3) Crear jugadores (SOLO si trae nombre + CURP + foto)
    if (createdTeamId) {
      for (const p of players.value) {
        const hasName = !!p.fullName?.trim()
        const hasCurp = !!p.curp?.trim()
        const hasPhoto = !!p.photoFile

        // ✅ Si le falta algo: NO se registra
        if (!hasName || !hasCurp || !hasPhoto) continue
  ...
  <main class="bg-[#F3F4FF] text-slate-900 min-h-screen pt-24 md:pt-28 lg:pt-32">
    <div class="max-w-5xl mx-auto container-pad px-6 pb-16">
      ...
      <div v-else class="mt-6 space-y-4">
        ...
        <div v-else-if="myTeams && !myTeams.canCreateTeam"
            ...
            <div class="space-y-4">
              ...
              <div class="grid md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input v-model="teamShortName" type="text"
                    class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:border-blue-500/70"
                    placeholder="Ej. TIG" />
                </div>
              </div>
              ...
              <div class="grid md:grid-cols-4 gap-4 mt-2">
                ...
                <div>
                  ...
                  <select v-model.number="selectedSeasonId" :disabled="!selectedLeagueId"
                ...
                <div>
                  ...
                  <select v-model="selectedGender" :disabled="categoriesLoading || !selectedLeagueId"
                    class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs text-slate-900 disabled:bg-slate-100 disabled:text-slate-400">
                ...
                <div>
                  ...
                  <select v-model="selectedRama" :disabled="!selectedGender"
                    class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs text-slate-900 disabled:bg-slate-100 disabled:text-slate-400">
                    <option value="" disabled>
              ...
              <div class="grid md:grid-cols-2 gap-4 mt-2">
                <div>
                  ...
                  <div class="flex items-center gap-3">
                    <input v-model="colorPrimary" type="color"
                      class="h-9 w-9 rounded-lg border border-slate-300 bg-white cursor-pointer" />
                    <input v-model="colorPrimary" type="text"
                ...
                <div>
                  ...
                  <div class="flex items-center gap-3">
                    <input v-model="colorSecondary" type="color"
                      class="h-9 w-9 rounded-lg border border-slate-300 bg-white cursor-pointer" />
                    <input v-model="colorSecondary" type="text"
                      class="flex-1 rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:border-blue-500/70"
              ...
              <div class="grid md:grid-cols-[minmax(0,2fr)_minmax(0,1fr)] gap-4 items-start mt-2">
                ...
                <div class="flex justify-center md:justify-end">
                  <div
                    ...
                    <img v-if="logoPreview" :src="logoPreview" alt="Logo del equipo"
              ...
              <div class="flex items-center justify-between gap-2">
                <div>
                  ...
                  <p class="text-xs text-slate-500">
                    ...
                    y
                    <strong>foto</strong>. Los integrantes incompletos <strong>no se enviarán</strong>.
                  </p>
                </div>
                <button type="button"
              ...
              <div class="space-y-3">
                <div v-for="(player, index) in players" :key="player.id"
                  ...
                  <div class="flex items-start justify-between gap-3 mb-3">
                    ...
                    <button v-if="players.length > 1" type="button"
                      class="text-[11px] text-slate-500 hover:text-red-500" @click="removePlayer(index)">
                      Eliminar
                    </button>
                  </div>
                  ...
                  <div class="grid md:grid-cols-[minmax(0,3fr)_minmax(0,2fr)] gap-4 items-start">
                    <div class="space-y-3">
                      ...
                      <div class="grid grid-cols-[minmax(0,2fr)_minmax(0,1fr)] gap-3">
                        <div>
                          ...
                          <input v-model="player.curp" type="text"
                        ...
                        <div>
                          ...
                          <input v-model.number="player.jerseyNumber" type="number" min="0"
                            class="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/70 focus:border-blue-500/70"
                            placeholder="Ej. 10" />
                        </div>
                      </div>
                    ...
                    <div class="space-y-2">
                      ...
                      <div
                        ...
                        <img v-if="player.photoPreview" :src="player.photoPreview" alt="Foto del jugador"
